...
export default {
  ...
  computed: {
    latestNews() {
      console.log("Computed: latestNews", this.articles);
      return this.articles.slice(0, 5);
    },
    visibleArticles() {
  console.log("Текущее значение articles:", this.articles);
  if (this.articles && Array.isArray(this.articles)) {
    return this.articles.slice(0, this.visibleArticlesCount);
  }
  console.warn("articles либо не определен, либо не массив");
  return [];
}

    
  //   bleArticles() {
  // console.log("Начало функции visibleArticles()");
  // console.log("Текущее значение articles:", this.articles);
  // console.log("Текущий тип articles:", typeof this.articles);

  // if (Array.isArray(this.articles)) {
  //   console.log("articles является массивом, возвращаем срез:");
  //   console.log("visibleArticlesCount:", this.visibleArticlesCount);
  //   return this.articles.slice(0, this.visibleArticlesCount);
  // }

  // console.log("articles не является массивом, возвращаем пустой массив.");
  // return [];
...
mounted() {
  console.log("Массив articles при загрузке компонента:", this.articles);
  console.log("Component mounted. Initial articles:", this.articles);

  // Эмуляция получения данных
  setTimeout(() => {
    this.articles = [
      { title: "Article 1", image: "/path/to/image1.jpg", content: "Content 1" },
      { title: "Article 2", image: "/path/to/image2.jpg", content: "Content 2" },
    ];
    console.log("Articles fetched:", this.articles);
    this.visibleArticles = this.articles.slice(0, this.visibleArticlesCount);
    console.log("Visible articles after fetch:", this.visibleArticles);
...
watch: {
    articles(newVal) {
      console.log("Articles updated:", newVal);
      this.visibleArticles = newVal.slice(0, this.visibleArticlesCount);
      console.log("Visible articles updated:", this.visibleArticles);
    },
  },
  ...
  methods: {
    ...
    updateCardWidth() {
      ...
      if (slider) {
        ...
        slider.querySelectorAll('.news-card').forEach((card) => {
...
,
loadMoreArticles() {
      console.log("Before loadMoreArticles:", this.visibleArticles);
      this.visibleArticlesCount += 8;
      this.visibleArticles = this.articles.slice(0, this.visibleArticlesCount);
      console.log("After loadMoreArticles:", this.visibleArticles);
    },
    hideArticles() {
      console.log("Hiding articles. Previous visible count:", this.visibleArticlesCount);
      this.visibleArticlesCount = 8;
      this.visibleArticles = this.articles.slice(0, this.visibleArticlesCount);
      console.log("Articles after hiding:", this.visibleArticles);
    },
    selectArticle(index) {
      console.log("Selected article index:", index);
      console.log("Selected article:", this.articles[index]);
